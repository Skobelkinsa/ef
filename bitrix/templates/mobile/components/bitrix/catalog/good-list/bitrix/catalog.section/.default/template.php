...
'<div class="bx_size_scroller_container"><div class="bx_size"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list" style="width: '.$strWidth.';">';
				foreach ($arProp['VALUES'] as $arOneValue)
				{
					...
					$templateRow .= '<li data-treevalue="'.$arProp['ID'].'_'.$arOneValue['ID'].'" data-onevalue="'.$arOneValue['ID'].'" style="width: 50%;" title="'.$arOneValue['NAME'].'"><i></i><span class="cnt">'.$arOneValue['NAME'].'</span></li>';
				}
				$templateRow .= '</ul></div>'.
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?><div class="bx_catalog_item_title"><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a><?if($arItem['PROPERTIES']['NEWPRODUCT']['VALUE']):?><div class="new-product-block"></div><?endif;?>	</div>
...
<?
		...
		<div class="bx_catalog_item_controls no_touch">
			...
			{
			?>
		<span class="ibws kol-text ">Количество, шт</span>	
