			...
			<thead>
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						?>
						<?if ($arHeader["id"] != "NAME" && $arHeader["id"] != "PROPERTY_DOSAGE_VALUE"):?>
							<?=$arHeader["name"]; ?>
						<?endif;?>
							</td>
					<?
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
								...
								<td class="item">
									...
									<div class="ibws textdetail">
                                        <div class="bx_ordercart_itemart">
											...
											<?=$arItem["CATALOG"]["PARENT_PREVIEW_TEXT"]?>
										...
										<table cellspacing="0" cellpadding="0" class="counter">
											<tr>
												<td>
													...
													?>
													<a href="javascript:void(0);" class="minus valcontrol" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'down', <?=$useFloatQuantityJS?>);">-</a>
													<input
														type="text"
														size="3"
														...
														<?=$max?>
														step="<?=$ratio?>"
														class="valcon-mid<?if($arItem["QUANTITY"]>=2):?> red<?endif;?>"
														value="<?=$arItem["QUANTITY"]?>"
														onchange="updateQuantity('QUANTITY_INPUT_<?=$arItem["ID"]?>', '<?=$arItem["ID"]?>', <?=$ratio?>, <?=$useFloatQuantityJS?>)"
													>
													<a href="javascript:void(0);" class="plus valcontrol" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'up', <?=$useFloatQuantityJS?>);">+</a>
												</td>
												<?
							...
							?>
								<td class="price">
									...
									<?endif;?>
<div class="ibws del-buts">
<? if ($bDeleteColumn):?><a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"><?=GetMessage("SALE_DELETE")?></a><br />
	<?endif; if ($bDelayColumn):?>
<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?=GetMessage("SALE_DELAY")?></a><?endif;?></div>
								...
								<td class="custom tush">
									...
									if (is_array($arItem["SKU_DATA"]) && !empty($arItem["SKU_DATA"])):
										foreach ($arItem["SKU_DATA"] as $propId => $arProp):
											...
											?>
												<div class="bx_item_detail_size_small_noadaptive pr <?=$full?>">
	...
	<div class="bx_ordercart_order_pay">
		...
		<div class="bx_ordercart_order_pay_left" id="coupons_block">
			<div class="bx_ordercart_coupon">
				<span class="dl-steek"><?=GetMessage("STB_COUPON_PROMT")?></span><input type="text" id="coupon" name="COUPON" value="" onchange="enterCoupon();">&nbsp;<a class="bx_bt_button bx_big" href="javascript:void(0)" onclick="enterCoupon();" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"><?=GetMessage('SALE_COUPON_APPLY'); ?></a>
			</div><?
...
?>
		...
		<div class="bx_ordercart_order_pay_right">
			<table class="bx_ordercart_order_sum">
				...
				<?endif;?>
					<tr class="total-summ">
						<td class=""><?=GetMessage("SALE_TOTAL")?></td>
