...
<div class="login_page bx_<?=$arResult["THEME"]?>">
    ...
    ?>
    <div class="socail_auth">

    <?if($arResult["AUTH_SERVICES"]):?>
        <p><?echo GetMessage("AUTH_SERVICES_TITLE")?></p>
    ...
    <form name="form_auth" method="post" target="_top" action="<?=SITE_DIR?>auth/<?//=$arResult["AUTH_URL"]?>" class="bx_auth_form">
        ...
        <?if ($arResult["STORE_PASSWORD"] == "Y"):?>
            <label class="rememberme">
                ...
                <span class="label"><?=GetMessage("AUTH_REMEMBER_ME")?></span>
            </label>
        <?endif?>
        ...
        <?endif?>
        <br><br><input type="submit" name="Login" class="bx_bt_button bx_big shadow" value="<?=GetMessage("AUTH_AUTHORIZE")?>" /> <br><br>
        <p><a href="/login/?register=yes" class="reg">Регистрация</a></p>
    </form>
</div><script type="text/javascript"><?if (strlen($arResult["LAST_LOGIN"])>0):?>try{document.form_auth.USER_PASSWORD.focus();}catch(e){}<?else:?>try{document.form_auth.USER_LOGIN.focus();}catch(e){}<?endif?></script>