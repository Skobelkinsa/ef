    ...
    <div class="bx_catalog_item_block">
            ...
            if ($boolShowProductProps || $boolShowOfferProps)
            {
                ...
                <div class="bx_catalog_item_articul">
                    <?
                    foreach($arItem['OFFERS'] as $Offesr){
                        echo $Offesr["PROPERTIES"]["LVL"]["VALUE"];
                    }
                    ?>
                </div>
                <?
        ...
        <div class="bx_catalog_item_title"><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a>
            <?if($arItem['PROPERTIES']['NEWPRODUCT']['VALUE']):?><div class="new-product-block"></div><?endif;?>
            </div>
        <div class="bx_section_lister">
                <?php  $db_old_groups = CIBlockElement::GetElementGroups($arItem['ID'], false);
        ...
        <div class="bx_catalog_item_price">
            <div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price">
                <? echo "от ".$minPrice['PRINT_VALUE']; ?>
            </div>
            <?if($arItem["MIN_PRICE"]["DISCOUNT_DIFF_PERCENT"]>0):?>
                <div class="bx_catalog_item_sale">скидка -<?=$arItem["MIN_PRICE"]["DISCOUNT_DIFF_PERCENT"]?>%</div>
            <?endif;?>
        </div>
        <div class="bx_catalog_item_anonce">
            <?=$arItem["PREVIEW_TEXT"]?>
        </div>
<?
...
<?echo ('' != $arParams['MESS_NOT_AVAILABLE'] ? $arParams['MESS_NOT_AVAILABLE'] : GetMessage('CT_BCT_TPL_MESS_PRODUCT_NOT_AVAILABLE'));?>
		...
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">
            ...
            <a class="bx_bt_button bx_medium buy_link" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" rel="nofollow"><?=GetMessage('CT_BCT_TPL_MESS_BTN_DETAIL')?></a>
            <?/* add to ajax cart
            <a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium buy_link" href="javascript:void(0)" rel="nofollow">
                <?
			if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
