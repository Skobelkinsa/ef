...
foreach ($arResult['PROPERTIES']['BLOG']['VALUE'] as $kay => $blog_item_id):
    $arSelect = Array("ID", "NAME", "DETAIL_PAGE_URL");
    $arFilter = Array("IBLOCK_CODE"=>"furniture_news_s1", "ID"=>$blog_item_id);
    $res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
    while($ob = $res->GetNext())
    {
        $arResult['PROPERTIES']['BLOG']['VALUE'][$kay] = $ob;
    }
endforeach;

$arSelect = Array("ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_TEXT", "PREVIEW_PICTURE");
$arFilter = Array("IBLOCK_CODE"=>"furniture_news_s1", "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
$res = CIBlockElement::GetList(Array("RAND"=>"ASK"), $arFilter, false, Array("nPageSize"=>4), $arSelect);
$cnt = 0;
while($ob = $res->GetNext())
{
    $arResult['BLOG_RAND_ITEMS'][$cnt]["NAME"] = $ob["NAME"];
    $arResult['BLOG_RAND_ITEMS'][$cnt]["URL"] = "/news/".$ob["DETAIL_PAGE_URL"];
    $arResult['BLOG_RAND_ITEMS'][$cnt]["PREVIEW_TEXT"] = $ob["PREVIEW_TEXT"];
    $arResult['BLOG_RAND_ITEMS'][$cnt]["IMG"] = CFile::GetPath($ob["PREVIEW_PICTURE"]);
    $cnt++;
}